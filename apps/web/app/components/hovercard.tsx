import {tmdb,GetActivity} from "@/lib/api"
import {StarIcon} from "./icons/staricon"
interface HoverCardProps {
    id:string;
    mediaType:"movie"|"tv"
}
export async function HoverCard(props:HoverCardProps)
{
    
    const media = props.mediaType === "movie" ? await tmdb.getMovie(props.id) : await tmdb.getTv(props.id);

    const activity = await GetActivity(props.id, props.mediaType);

return (
  <div className="w-72 rounded-xl bg-accent p-5 transition-all duration-200 hover:scale-[1.03] hover:shadow-xl">

    
    <h2 className="text-center font-Ubuntu text-2xl font-medium text-background line-clamp-2">
      {media.title || media.name}
    </h2>

    
    <div className="mt-2 flex justify-between text-sm font-Ubuntu text-background/80">
      <p>{props.mediaType === "movie" ? media.release_date : media.first_air_date}</p>
